...
import { tokenizeText, type Token, queryWordWithComparison } from '../utils/wordService'
...
// 获取难度≥3单词的翻译（已废弃，使用getWordTranslationForDisplay）
const getDifficultWordTranslation = (word: string): string => {
  // 直接调用新的翻译显示函数
  return getWordTranslationForDisplay(word)
}
...
const fetchWordTranslation = async (word: string) => {
  ...
  try {
    ...
    // 只有难度级别 >= 3 的单词才显示翻译
    if (wordInfo && wordInfo.difficultyLevel >= 3 && wordInfo.definitions && wordInfo.definitions.length > 0) {
...
const processWordsWithAI = async (words: string[]) => {
  ...
  try {
    ...
    if (newWords.length > 0) {
      ...
      try {
        ...
        if (aiAnalysisResult.code === 'SUCCESS' && aiAnalysisResult.data) {
          const complexWords = aiAnalysisResult.data.complex_words_details || aiAnalysisResult.data.complexWords || []
          
          // 步骤4: 只将复杂单词（难度≥3）添加到缓存中
          complexWords.forEach(wordData => {
            const key = wordData.word.toLowerCase()
          ...
          if (import.meta.env.DEV) {
            ...
            // 输出前5个单词的详细信息用于调试
            complexWords.slice(0, 5).forEach(wordData => {
    ...
    } else if (error.message?.includes('Network Error') || error.message?.includes('网络连接失败')) {
