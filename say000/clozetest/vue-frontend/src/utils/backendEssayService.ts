...
// 新增：导入API_URLS，获取统一管理的路径（避免硬写/api）
...
export const processEssayWithBackend = async (text: string, title: string = '未命名文章'): Promise<EssayProcessResult> => {
  try {
    // 核心修改：用API_URLS.essay.process()替换硬写的'/api/essay/process'
    const response = await httpClient.post<BackendEssayProcessResponse>(
      API_URLS.essay.process(), 
