...
export const saveMeaningToDatabase = async (word: string, meaning: string): Promise<boolean> => {
  try {
    ...
    if (axiosError.response?.status === 400 && axiosError.response?.data?.message?.includes('已存在')) {
  
      return true
    }
    
    console.error('保存单词到数据库失败:', (error as any).response?.data || (error as any).message)
    return false
  }
}
