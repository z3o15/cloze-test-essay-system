...
export const splitCompoundWord = (word: string): string[] => {
  ...
  for (const pattern of COMPOUND_WORD_PATTERNS) {
    ...
    if (match) {
      const parts = [match[1], match[2]].filter(part => part)
      if (parts.length === 2) {
        return parts
      }
    }
...
export const extractValidWords = (text: string): string[] => {
  ...
  // 使用改进的正则表达式提取单词，避免单字母片段
  // 匹配：3个或以上字母的单词，或者在白名单中的短单词
  const words = text.match(/\b[a-zA-Z']{3,}\b/g) || []
  
  // 额外提取白名单中的短单词
  const shortWordPattern = new RegExp(`\\b(${Array.from(SHORT_WORD_WHITELIST).join('|')})\\b`, 'gi')
  const shortWords = text.match(shortWordPattern) || []
  
  // 合并所有单词
  const allWords = [...words, ...shortWords]
  
  // 过滤有效单词
  return filterValidWords(allWords)
}
 ...
 */
export const isBasicWord = (_word: string): boolean => {
