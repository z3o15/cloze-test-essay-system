    ...
    try {
      ...
      for (let i = 0; i < batches.length; i++) {
        ...
        try {
          ...
          // 尝试重试失败的批次（减少批次大小）
          let retryResult: WordDifficultyAnalysisResponse | null = null;
          if (batches[i].length > 1) {
            ...
            try {
              ...
              // 合并重试结果（修复：使用当前小批次的总长度，而非外层batches[i].length）
              retryResult = this.mergeBatchResults(retryResults, smallBatches.length);
              
              // 检查重试结果并输出日志（修复：判空逻辑）
              if (retryResult) {
                console.log(`✅ [重试成功] 批次重试完成，成功处理 ${retryResult.data.total_words} 个单词`);
              } else {
                console.log(`ℹ️ [重试结果为空] 批次重试未返回有效数据`);
              }
