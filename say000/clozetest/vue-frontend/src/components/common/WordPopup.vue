...
// 获取难度级别对应的CSS类
const getDifficultyClass = (level: number | undefined): string => {
  if (!level || level <= 2) return 'difficulty-easy'
...
// 获取难度级别描述
const getDifficultyDescription = (level: number | undefined): string => {
  if (!level || level <= 2) return '简单'
  if (level <= 4) return '中等'
  if (level <= 6) return '困难'
  if (level <= 8) return '很困难'
  return '极难'
}
