...
export class SimplifiedWordController {
      ...
      res.json({
        ...
        data: {
          ...
          stats: {
            ...
            complexCount: result.words.filter((w: any) => w.isComplex).length,
            simpleCount: result.words.filter((w: any) => !w.isComplex).length
          }
        }
      });
   ...
   */
  static async checkDisplayNeeded(req: Request, res: Response): Promise<void> {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
        res.status(400).json({
   ...
   */
  static async filterComplexWords(req: Request, res: Response): Promise<void> {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
        res.status(400).json({
   ...
   */
  static async processSingleWord(req: Request, res: Response): Promise<void> {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
         res.status(400).json({
